...
const  Person::SexType::Male(0);
const  Person::SexType::Female(1);
...
const  Person::Sex::Male(0);
const  Person::Sex::Female(1);
...
const litesql::FieldType Person::Id("id_","",table__);
const litesql::FieldType Person::Type("type_","",table__);
const litesql::FieldType Person::Name("name_","",table__);
const litesql::FieldType Person::Age("age_","",table__);
std::vector < std::pair< std::string, std::string > > Person::sex_values;
const Person::SexType Person::Sex("sex_","",table__,sex_values);
const Person::SexType Person::DateOfBirth("dateOfBirth_","",table__);
...
Person::Person(const litesql::Database& db, const litesql::Record& rec)
    ...
    switch(size) {
    case 6: dateOfBirth = convert<const std::string&, >(rec[5]);
        dateOfBirth.setModified(false);
    case 5: sex = convert<const std::string&, >(rec[4]);
        sex.setModified(false);
    case 4: age = convert<const std::string&, >(rec[3]);
        age.setModified(false);
    case 3: name = convert<const std::string&, >(rec[2]);
        name.setModified(false);
    case 2: type = convert<const std::string&, >(rec[1]);
        type.setModified(false);
    case 1: id = convert<const std::string&, >(rec[0]);
        id.setModified(false);
    }
}
Person::Person(const Person& obj)
     : (obj), id(obj.id), type(obj.type), name(obj.name), age(obj.age), sex(obj.sex), dateOfBirth(obj.dateOfBirth) {
}
...
std::string Person::insert(litesql::Record& tables, litesql::Records& fieldRecs, litesql::Records& valueRecs) {
    ...
    return ::insert(tables, fieldRecs, valueRecs, sequence__);
}
void Person::create() {
...
const litesql::FieldType Role::Id("id_","",table__);
const litesql::FieldType Role::Type("type_","",table__);
void Role::defaults() {
}
...
const litesql::FieldType School::Id("id_","",table__);
const litesql::FieldType School::Type("type_","",table__);
const litesql::FieldType School::Name("name_","",table__);
...
School::School(const School& obj)
     : (obj), id(obj.id), type(obj.type), name(obj.name) {
}
...
const litesql::FieldType Office::Id("id_","",table__);
const litesql::FieldType Office::Type("type_","",table__);
...
std::vector<litesql::Database::SchemaItem> ExampleDatabase::getSchema() const {
    ...
    res.push_back(Database::SchemaItem("Person_","table","CREATE TABLE Person_ (id_ " + backend->getRowIDType() + ",type_ ,name_ ,age_ ,sex_ ,dateOfBirth_ )"));
    res.push_back(Database::SchemaItem("Role_","table","CREATE TABLE Role_ (id_ " + backend->getRowIDType() + ",type_ )"));
    ...
    res.push_back(Database::SchemaItem("School_","table","CREATE TABLE School_ (id_ " + backend->getRowIDType() + ",type_ ,name_ )"));
    res.push_back(Database::SchemaItem("Office_","table","CREATE TABLE Office_ (id_ " + backend->getRowIDType() + ",type_ )"));
