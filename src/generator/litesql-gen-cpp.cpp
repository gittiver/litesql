...
static void sanityCheck(const xml::DatabasePtr& db,
...
static void writeStaticObjData(Class& cl, const xml::Object& o) {
...
static void writeObjFieldValues(const xml::Object & o,const Field & fld, Class& cl, Method& initValues) {
   ...
   if (!fld.values.empty()) {
      ...
      for (size_t i2 = 0; i2 < fld.values.size(); i2++) {
         ...
         initValues.body(fld.name + "_values.push_back(make_pair<std::string, std::string>("
            + quote(val.name) + "," + quote(val.value) + "));");
      }
      string ftypeClass = fld.fieldTypeName + "Type";
...
static void writeObjFields(Class & cl, const xml::Object & o)
{
   ...
   for (size_t i = 0; i < o.fields.size(); i++) {
      ...
      hasValues = hasValues||fld.values.empty();
      writeObjFieldValues(o,fld,cl,init);
      ...
      if (fld.values.size() > 0) {
         Class valueHolder(litesql::capitalize(fld.name));
...
static void writeObjConstructors(Class& cl, const xml::Object& o) {
...
static void writeObjRelationHandles(Class& cl, xml::Object& o) {
...
static void writeObjBaseMethods(Class& cl, const xml::Object& o) {
    ...
    if (o.parentObject.get()) {
        ...
        if (o.parentObject->parentObject.get())
    ...
    Method upcast("upcast", "std::unique_ptr<" + o.name + ">");
    upcast.isConst = true;

    Method upcastCopy("upcastCopy", "std::unique_ptr<" + o.name + ">");
...
static void writeStaticRelData(Class& cl, const xml::Relation& r) {
    ...
    for (size_t i2 = 0; i2 < r.fields.size(); i2++) {
        ...
        if (!fld.values.empty()) {
            ...
            cons.param(Variable("n", "const std::string&"))
                .param(Variable("t", "AT_field_type"))
                .param(Variable("tbl", "const std::string&"))
                .param(Variable("vals", "const litesql::FieldType::Values&", "Values()"))
                .constructor("litesql::FieldType(n,t,tbl,vals)");
...
//        .constructor("litesql::Record(db, rec)");
    ...
    for (int i = r.related.size()-1; i >= 0; i--) {
        ...
        string fname = decapitalize(rel.fieldTypeName);
...
static void writeRelMethods(const xml::DatabasePtr& database,
...
static void getSchema(const xml::DatabasePtr& db, Method& mtd) {
    vector<string> rec;
...
static void writeDatabaseClass(FILE* hpp, FILE* cpp,
                        const xml::DatabasePtr& dbInfo,
                        const ObjectSequence& objects,
                        const Relation::sequence& relations) {
    ...
    db.method(cons);
    Method getSchemaMtd("getSchema", "std::vector<litesql::Database::SchemaItem>");
...
void CppGenerator::writeCPPClasses(const ObjectModel* model)
{
    ...
    for (Relation::sequence::const_iterator it = model->relations.begin(); it!= model->relations.end(); it++) {
        Class cl((*it)->getName());
    ...
    for (ObjectSequence::const_iterator it_o=model->objects.begin(); it_o!=model->objects.end(); it_o++) {
        ...
        Method strMtd("operator<<", "std::ostream &");
