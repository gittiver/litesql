...
static string getMethodName(xml::Relate* relate) {
    if (relate->relation->maxSameTypes() == 1)
        return "get" + brackets2(relate->objectName);
    return "get" + relate->objectName + toString(relate->sameObjectPos);
}
void writeStaticObjData(Class& cl, xml::Object& o) {
    ...
    for (size_t i = 0; i < o.fields.size(); i++) {
        ...
        if (!fld.values.empty()) {
            ...
            for (size_t i2 = 0; i2 < fld.values.size(); i2++) {
                ...
                init.body(fld.name 
                     + "_values.push_back(make_pair<std::string, std::string>("
                     + quote(val->name) + "," 
                     + quote(val->value) + "));");    
            }
        }
        ...
        ftype.static_();
        Variable field(fld.name, "litesql::Field<" + fld.getClass("c++") + ">");
        cl.variable(ftype);
        cl.variable(field);
        if (fld.values.size() > 0) {
            Class valueHolder(xml::capitalize(fld.name));
            for (size_t v = 0; v < fld.values.size(); v++) {
                xml::Value* value = fld.values[v];
                string v;
                if (fld.getClass("c++") == "std::string")
                    v = quote(value->value);
                else
                    v = value->value;
                Variable val(value->name, "const " + fld.getClass("c++"), v);
...
void writeObjConstructors(Class& cl, xml::Object& o) {
    Method defaults("defaults", "void");
    defaults.protected_();
    bool hasDefaults = false;
    for (size_t i = 0; i < o.fields.size(); i++) {
        xml::Field& f = *o.fields[i];
    ...
    string consParams = getInherits(o) + "(db)";
    string cons2Params = getInherits(o) + "(db, rec)";
    if (o.fields.size() > 0) {
        Split fieldInst;
        for (size_t i = 0; i < o.fields.size(); i++) {
            xml::Field& f = *o.fields[i];
    ...
    string consParams3 = getInherits(o) + "(obj)";    
    if (o.fields.size() > 0) {
        Split fieldCopy;
        for (size_t i = 0; i < o.fields.size(); i++) {
            xml::Field& f = *o.fields[i];
    ...
    assign.body(getInherits(o) + "::operator=(obj);");
...
void writeObjRelationHandles(Class& cl, xml::Object& o) {
    ...
    for (size_t i = 0; i < o.handles.size(); i++) {
        ...
        for (size_t i2 = 0; i2 < rel->fields.size(); i2++) {
            ...
            link.param(Variable(field.name, field.getClass("c++"), 
                                field.getQuotedDefaultValue()));
            unlink.param(Variable(field.name, field.getClass("c++")));
            params.push_back(field.name);
        }
        link.body(rel->name + "::link(" + params.join(", ") + ");");
        unlink.body(rel->name + "::unlink(" + params.join(", ") + ");");
        string exprType = "Expressions::" + rel->name;
        Variable srcExprParam("srcExpr", "const " + exprType + "&", 
                exprType + "()");
        Method del("del", "void");
        params.clear();
        params.push_back("owner->getDatabase()");
        params.push_back("srcExpr && " + rel->name 
                + "::" + handle.relate->fieldTypeName + " == owner->id");
        del.param(srcExprParam).body(rel->name + "::del(" + params.join(", ") + ");");
        ...
        if (handle.destObjects.size() == 1) {
            ...
            params.push_back("(" + rel->name + "::" + handle.relate->fieldTypeName + " == owner->id)"
                             + extraExpr);
            
            get
                .body("return " + rel->name + "::" + getMethodName(relate)
                      + brackets(params.join(", ")) + ";");
            
            hcl.method(get);
        } else {
            if (rel->maxSameTypes() <= 2) {
                ...
                for (size_t i2 = 0; i2 < handle.destObjects.size(); i2++) {
                    ...
                    params.push_back("(" + rel->name + "::" + handle.relate->fieldTypeName + " == owner->id)"
                                     + extraExpr);
                    get.body("return " + rel->name + "::" 
                                + getMethodName(relate)
                              + brackets(params.join(", ")) + ";");
                    hcl.method(get); 
                }    
            } else {
        ...
        Method getRows("getRows", "litesql::DataSource<" 
                        "RowTypes::" + rel->name + "Row>");
        getRows.param(srcExprParam)
            .body("return " + rel->name
...
void writeObjBaseMethods(Class& cl, xml::Object& o) {
    ...
    if (o.parentObject) {
        insert.body("return " + getInherits(o) 
    ...
    if (o.parentObject) 
        addUpdates.body(getInherits(o) + "::addUpdates(updates);");
    ...
    if (o.parentObject) {
        ...
        if (o.parentObject->parentObject)
            addIDUpdates.body(getInherits(o) + "::addIDUpdates(updates);");
    }
    ...
    if (o.parentObject)
        delRecord.body(getInherits(o) + "::delRecord();");
    ...
    for (map<xml::Relation*, vector<xml::Relate*> >::const_iterator i = 
        ...
        for (size_t i2 = 0; i2 < relates.size(); i2++)
            params.push_back("(" + rel->name + "::" + relates[i2]->fieldTypeName
                             + " == id)");
        delRelations.body(rel->name + "::del(*db, " + params.join(" || ") + ");");
    }
...
void writeRelRowType(Class& cl, xml::Relation& r) {
    string name = r.name + "Row";
    Class rowcl(name);
    Method rowcons(name);
...
//        .constructor("litesql::Record(db, rec)");
    ...
    for (int i = r.fields.size()-1; i >= 0; i--) {
        ...
        Variable fldvar(fld.name, "litesql::Field<" + fld.getClass("c++") + ">");
...
void writeStaticRelData(Class& cl, xml::Relation& r) {
    ...
    for (size_t i2 = 0; i2 < r.fields.size(); i2++) {
        ...
        Variable ftype(fld.fieldTypeName, r.name + "::FieldType", data);
...
void writeRelMethods(xml::Database& database,
    ...
    Variable srcExpr("srcExpr", "const Expressions::" + r.name + "&", 
                        "Expressions::" + r.name + "()");
    Method link("link", "void");
    Method unlink("unlink", "void");
    Method del("del", "void");
    Method getRows("getRows", 
                   "litesql::DataSource<"+r.name+"::Row>");
    ...
    Variable destExpr("expr", "const " + r.name + "::Expr&", 
                       r.name + "::Expr()");
    ...
    getRows.body("sel.source(table__);")
        .body("sel.where(expr);")
        .body("return DataSource<" + r.name + "::Row>(db, sel);");
    ...
    for (size_t i2 = 0; i2 < r.fields.size(); i2++) {
        xml::Field& fld = *r.fields[i2];
        link.param(Variable(fld.name, fld.getClass("c++"), 
                            fld.getQuotedDefaultValue()));
        unlink.param(Variable(fld.name,  fld.getClass("c++")));
    }
...
void getSchema(xml::Database& db,
...
//    recs.push_back(rec);
    ...
    for (size_t i = 0; i < db.tables.size(); i++) {
        ...
        rec.push_back(quote("table"));
        // FIXME: rec.push_back(quote(db.tables[i]->getSQL("\" + backend->getRowIDType() + \"")));
        mtd.body("res.push_back(Database::SchemaItem(" + rec.join(",") + "));");
    }
...
void writeCPPClasses(xml::Database& db, Args& args) {
    ...
    for (size_t i = 0; i < relations.size(); i++) {
        xml::Relation & r = *relations[i];
        writeExpr(exprcl, "Expressions::" + r.name, r.name);
    ...
    for (size_t i = 0; i < relations.size(); i++) {
        ...
        cl.typedef_("Row", "RowTypes::" + o.name + "Row");

        writeFieldType(cl, "Expressions::" + o.name, 
                o.name + "::FieldType");
